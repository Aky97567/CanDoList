...
export class FirebaseAdapter implements TaskStorage {
  ...
  async getTasks(): Promise<Task[]> {
    try {
      console.log('FirebaseAdapter: Fetching tasks from path:', this.getCollectionPath());
      ...
      console.log('FirebaseAdapter: Got snapshot, document count:', snapshot.size);
  ...
  async updateTaskRanks(taskIds: string[], ranks: string[]): Promise<void> {
    try {
      ...
      taskIds.forEach((taskId, index) => {
        console.log(`FirebaseAdapter: Setting rank ${ranks[index]} for task ${taskId}`);
